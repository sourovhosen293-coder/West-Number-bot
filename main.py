...
async def stats_page(query, user_id):
    ...
    if user["subscription"]:
        sub = custom_emoji(EMOJI["active"], "🟢") + " Active"
    else:
        sub = custom_emoji(EMOJI["inactive"], "🔴") + " Detective"
    ...
    text = (
        ...
        f"<b>Subscription Price:</b> {user['subscription_price']}\n\n"
        f"{custom_emoji(EMOJI['duration'], '⏳')} "
        f"<b>Duration:</b> {user['duration']}\n\n"
        f"{custom_emoji(EMOJI['total_earning'], '💰')} "
        f"<b>Total Earning:</b> {user['total_earning']}\n\n"
        f"{custom_emoji(EMOJI['balance'], '💳')} "
        f"<b>My Balance:</b> {user['balance']:.2f} Tk"
    )
...
async def withdraw_page(query):
    keyboard = [
        [InlineKeyboardButton("💳 Nagad", callback_data="withdraw:Nagad")],
        [InlineKeyboardButton("💳 Rocket", callback_data="withdraw:Rocket")],
        [InlineKeyboardButton("💳 Binnace", callback_data="withdraw:Binance")],
...
async def support_page(query):
    rows = db.execute("SELECT * FROM supports ORDER BY id").fetchall()
    buttons = []

    for row in rows:
        value = row["value"]
        if value.startswith("https://t.me/"):
            buttons.append([InlineKeyboardButton(f"🆘 {value}", url=value)])
        elif value.startswith("@"):
            buttons.append([InlineKeyboardButton(f"🆘 {value}", url=f"https://t.me/{value[1:]}")])
        else:
            buttons.append([InlineKeyboardButton(f"🆘 {value}", callback_data="support_info")])

    if not buttons:
        buttons.append([InlineKeyboardButton("⚠️ Support not configured", callback_data="nothing")])
...
async def admin_page(query):
    ...
    keyboard = [
        ...
        [
            InlineKeyboardButton("🆘 Support", callback_data="admin_support"),
            InlineKeyboardButton("🚫 Ban / Unban", callback_data="admin_ban")
        ],
        [InlineKeyboardButton("📱 Get Number", callback_data="admin_services")],
        [InlineKeyboardButton("🌍 Country Manager", callback_data="admin_countries")],
...
async def getnumber_set(update, context):
    ...
    names = re.split(r"[\s,]+", parts[1])
    added = []

    for name in names:
        key = ALIASES.get(name.lower().strip())
        if not key:
            continue
        db.execute("UPDATE services SET enabled=1 WHERE service=?", (key,))
        added.append(SERVICES[key]["name"])
    ...
    if added:
        ...
        await update.message.reply_text("❌ No valid service found.")
...
async def getnumber_remove(update, context):
    if not is_admin(update.effective_user.id):
        return
    text = update.message.text
    match = re.match(r"^/getnumber_remove_(.+)$", text, re.I)
    if not match:
        return
    ...
    db.execute("UPDATE services SET enabled=0 WHERE service=?", (key,))
    db.commit()

    await update.message.reply_text(
        f"✅ <b>{SERVICES[key]['name']}</b> removed from Get Number.",
        parse_mode=ParseMode.HTML
    )
...
async def add_country(update, context):
    if not is_admin(update.effective_user.id):
        return
    text = update.message.text
    match = re.match(r"^/getnumber_([A-Za-z0-9]+)_country\s+(.+)$", text, re.I | re.S)

    if not match:
        await update.message.reply_text("Example:\n\n/getnumber_instagram_country 🇵🇸 Sudan - 0.8Tk/OTP")
        return

    service_name_input = match.group(1)
    country = match.group(2).strip()
    service = ALIASES.get(service_name_input.lower())

    if not service:
        await update.message.reply_text("❌ Service not found.")
        return

    db.execute("INSERT INTO countries (service, country) VALUES (?, ?)", (service, country))
    db.commit()
...
async def remove_country(update, context):
    if not is_admin(update.effective_user.id):
        return
    text = update.message.text
    match = re.match(r"^/getnumber_country_(.+)_remov$", text, re.I | re.S)
    if not match:
        return

    country = match.group(1).strip()
    cursor = db.execute("DELETE FROM countries WHERE country=?", (country,))
    db.commit()

    if cursor.rowcount:
        await update.message.reply_text("✅ Country removed successfully.")
    else:
        await update.message.reply_text("⚠️ Country not found.")
...
async def set_userstats(update, context):
    ...
    first_line = lines[0]
    match = re.match(r"^/set_userstasts_(\d+)$", first_line.strip(), re.I)
    ...
    for line in lines[1:]:
        ...
        if key == "subscription":
            data["subscription"] = 1 if "active" in value.lower() else 0
            ...
            data["balance"] = float(number) if number else 0
...
async def support_setup(update, context):
    ...
    await update.message.reply_text(
        ...
        "Example: <code>3</code>\n\n"
        "তারপর একে একে username / ID / profile link পাঠাবেন।",
        parse_mode=ParseMode.HTML
    )


async def support_setup_message(update, context):
    if not is_admin(update.effective_user.id) or not context.user_data.get("support_setup"):
        return False
    ...
    if context.user_data["support_count"] is None:
        if not text.isdigit():
            await update.message.reply_text("❌ শুধু সংখ্যা দিন। Example: 3")
            return True
        count = int(text)
        if count < 1 or count > 10:
            await update.message.reply_text("❌ 1 থেকে 10-এর মধ্যে দিন।")
            return True

        context.user_data["support_count"] = count
        await update.message.reply_text(f"✅ {count}টি Support ID লাগবে।\n\nএখন প্রথম Support ID পাঠান।")
        return True

    values = context.user_data["support_values"]
    values.append(text)
    count = context.user_data["support_count"]
    current = len(values)

    if current < count:
        await update.message.reply_text(f"✅ Saved {current}/{count}\nNext Support ID পাঠান।")
        return True

    db.execute("DELETE FROM supports")
    for value in values:
        db.execute("INSERT INTO supports(value) VALUES(?)", (value,))
    db.commit()
...
def parse_number(text):
    ...
    service, service_index = None, -1
    for i, part in enumerate(parts[:7]):
        key = ALIASES.get(part.lower().strip(":-"))
        if key:
            service, service_index = key, i
            break
    ...
    number = None
    for part in parts[service_index + 1:]:
    ...
    country = " ".join(parts[:service_index])
    return country, service, number


async def incoming_number(update, context):
    user = update.effective_user
    ensure_user(user)
    row = get_user(user.id)
    if row["banned"]:
    ...
    await update.message.reply_text(
        f"{country} {emoji} <b>{label}</b> <code>{number}</code>\n\n"
...
async def callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    row = get_user(user.id)
    if row["banned"]:
        await query.edit_message_text("🚫 <b>You are banned.</b>", parse_mode=ParseMode.HTML)
        return
    ...
    if data == "home":
        ...
        await get_number_page(query)

    elif data.startswith("service:"):
        service = data.split(":", 1)[1]
        if service in SERVICES:
            await service_page(query, service)

    elif data.startswith("country:"):
        country_id = int(data.split(":", 1)[1])
        country = db.execute("SELECT * FROM countries WHERE id=?", (country_id,)).fetchone()
        if not country:
            await query.answer("Country not found.", show_alert=True)
            return

        if not row["subscription"]:
            await query.answer("Your subscription not active ❌", show_alert=True)
            await query.edit_message_text(
                ...
                reply_markup=InlineKeyboardMarkup([
                    ...
                    [InlineKeyboardButton("🔙 Back", callback_data=f"service:{country['service']}")]
                ])
            )
            return
        ...
        await stats_page(query, user.id)

    elif data == "withdraw":
        await withdraw_page(query)

    elif data.startswith("withdraw:"):
        await query.answer(f"Minimum Withdraw {MIN_WITHDRAW} Tk", show_alert=True)

    elif data == "support":
        await support_page(query)

    elif data == "support_info":
        await query.answer("Please use the support contact.", show_alert=True)
        ...
        active = db.execute("SELECT COUNT(*) c FROM users WHERE subscription=1").fetchone()["c"]
        await query.edit_message_text(
            "📊 <b>BOT STATISTICS</b>\n\n"
        ...
        text = "📱 <b>GET NUMBER SERVICES</b>\n\n"
        for key in SERVICES:
            status = "🟢 ON" if key in active else "🔴 OFF"
            text += f"{status} {SERVICES[key]['name']}\n"
        text += "\n<b>Commands:</b>\n\n<code>/getnumber_set Instagram Facebook WhatsApp</code>\n\n<code>/getnumber_remove_Facebook</code>"
        ...
        await query.edit_message_text(
            "🌍 <b>COUNTRY MANAGER</b>\n\n"
            "<b>Add Country:</b>\n<code>/getnumber_instagram_country 🇵🇸 Sudan - 0.8Tk/OTP</code>\n\n"
            "<b>Remove Country:</b>\n<code>/getnumber_country_🇵🇸 Sudan - 0.8Tk/OTP_remov</code>",
            parse_mode=ParseMode.HTML,
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Admin Panel", callback_data="admin")]])
        )
        ...
        if not is_admin(user.id):
            return
        supports = db.execute("SELECT value FROM supports ORDER BY id").fetchall()
        current = "\n".join(f"{i+1}. {x['value']}" for i, x in enumerate(supports)) or "No support configured."
        await query.edit_message_text(
            f"🆘 <b>SUPPORT MANAGER</b>\n\n{current}\n\nTo configure:\n<code>/support_setup</code>",
            parse_mode=ParseMode.HTML,
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Admin Panel", callback_data="admin")]])
        )
...
async def text_router(update, context):
    if await support_setup_message(update, context):
        return
    await incoming_number(update, context)


def main():
    if not BOT_TOKEN or BOT_TOKEN == "PASTE_YOUR_BOT_TOKEN_HERE":
        print("❌ BOT_TOKEN বসানো হয়নি!")
        return

    if not ADMIN_IDS:
        print("❌ ADMIN_IDS বসানো হয়নি!")
        return

    print("================================")
    print("   SECRET NUMBER BOT STARTING")
    print("================================")

    # Render এ Connect Timeout সমস্যা সমাধানের জন্য Timeout টাইম বাড়িয়ে দেওয়া হলো
    app = (
        Application.builder()
        .token(BOT_TOKEN)
    ...
    app.add_handler(CommandHandler("getnumber_set", getnumber_set))
    app.add_handler(CommandHandler("ban", ban))
    app.add_handler(CommandHandler("unban", unban))
    app.add_handler(CommandHandler("broadcast", broadcast))
    app.add_handler(CommandHandler("support_setup", support_setup))

    # Special commands
    app.add_handler(MessageHandler(filters.Regex(r"^/getnumber_remove_.+"), getnumber_remove))
    app.add_handler(MessageHandler(filters.Regex(r"^/getnumber_[A-Za-z0-9]+_country\s+.+"), add_country))
    app.add_handler(MessageHandler(filters.Regex(r"^/getnumber_country_.+_remov$"), remove_country))
    app.add_handler(MessageHandler(filters.Regex(r"^/set_userstasts_\d+"), set_userstats))
